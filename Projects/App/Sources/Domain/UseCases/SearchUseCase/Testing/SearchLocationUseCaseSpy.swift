...
public final class SearchLocationUseCaseSpy: SearchLocationUseCaseProtocol {
    ...
    private func getAddress(keyword: String) -> Observable<[Address]> {
        let add1 = Address(id: UUID().uuidString, name: "서면역", address: "진구엿나 어디야", lat: 0.0, lng: 0.0)
        let add2 = Address(id: UUID().uuidString, name: "대연역", address: "지금여기", lat: 0.0, lng: 0.0)
        let add3 = Address(id: UUID().uuidString, name: "해운대역", address: "해운대구", lat: 0.0, lng: 0.0)
